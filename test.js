...

{
    ...
    assertInstanceOf(ac.signal, AbortSignal, 'abortController.signal was not instanceof AbortSignal');
    assertInstanceOf(ac.signal, EventTarget, 'AbortSignal was not instanceof EventTarget');
}
...

{
    ...
    signal.onabort = (e) => {
        ...
        assert(e.type === 'abort', 'onabort should get an event with the type "abort"');
        onabortCalled++;
    };
    ...
    assert(onabortCalled === 1, `onabort should be called once. It was called ${onabortCalled} times.`);
}

{
    ...
    assert(signal.aborted === false, 'Manual event dispatch should not flag signal as aborted');
    assert(onabortCalled === 2, `onabort should be called twice. It was called ${onabortCalled} times.`);
}

{
    ...
    assert(signal.aborted === true, 'Signal should be flagged as aborted.');
    assert(handlerCalled === 1, `handler should be called once. It was called ${handlerCalled} times.`);    
}

{
    ...
    assert(signal.aborted === true, 'Signal should be flagged as aborted.');
    assert(handlerCalled === 0, `Removed handler should not be called. It was called ${handlerCalled} times.`);   
...

{
    ...
    const listener = {
        handleEvent: function (e) {
            ...
            assert(e.type === 'abort', 'The type of event passed to handleEvent should be "abort"');
            assert(this === listener, 'The context of the handleEvent call should not change');
            this.calls++;
        },
        calls: 0
    };
    ...
    assert(signal.aborted === true, 'Signal should be flagged as aborted.');
    assert(listener.calls === 1, `handler should be called once. It was called ${listener.calls} times.`);    
}

{
    ...
    ac1.abort();
    assert(order1.join(',') === ['onabort', 'addEventListener'].join(','), 'In this case, onabort should fire before the addEventListener abort');
    ...
    ac2.abort();
    assert(order2.join(',') === ['addEventListener', 'onabort'].join(','), 'In this case, addEventListener abort should fire before the onabort');
}
